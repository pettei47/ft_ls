...
cd ${TEST_DATA_DIR}
ls ${OPTION} textfile.txt &> ${EXPECTS_DIR}/${TEST_NAME}
...
valgrind --log-file="${LOGS_DIR}/${TEST_NAME}_leaks.log" --leak-check=full --error-exitcode=1 ../../ft_ls ${OPTION} textfile.txt &> /dev/null
if [ $? -eq 0 ]; then
